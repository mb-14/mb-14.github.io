...
;( function( $, window, undefined ) {
	...
	$.Baraja.prototype = {
		...
		_initEvents : function() {
			...
			this.$el.on( 'click.baraja', 'li', function() {
				...
				if( !self.isAnimating ) {
					...
					var active=$('.box')[$("#baraja-el li").index(this)];
					$('.box').hide();
                    $(active).show();
				}
		...
		_navigate : function( dir ) {
			...
			this._applyTransition( $item, { transform : 'translateY(' + translation + 'px) rotate(' + rotation + 'deg)' }, function() {

				$item.off( self.transEndEventName );
				self._updateStack( $item, dir );

				self._applyTransition( $item, { transform : 'translateY(0px) rotate(0deg)' }, function() {
		...
		_fan : function( settings ) {
			...
			this.$items.each( function( i ) {
				...
				self._applyTransition( $el, { transform : 'translateY(' + position + 'px) rotate(' + angle + 'deg)' }, function() {
		...
		_add : function( $elems ) {
			...
			// animate new items
			$elems.css( 'transform', 'scale(1.8) translateY(200px) rotate(15deg)' ).reverse().each( function( i ) {
